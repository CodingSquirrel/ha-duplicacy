...
class LogParser:
    ...
    progress_re = re.compile('(?:Uploaded|Skipped) chunk \d+ size \d+, (?P<speed>\d+[\d.,]*\d*)MB/s (?P<hour>\d{2}):(?P<min>\d{2}):(?P<sec>\d{2}) (?P<percent>\d+[\d.,]*\d*)%')
    ...
    def convert_size(self, size, unit):
        UNITS = {None: 1, "K": 2 ** 10, "M": 2 ** 20, "G": 2 ** 30, "T": 2 ** 40}
        return int(float(size.replace(',','')) * UNITS[unit])
    ...
    def parse_backup_stats(self, line):
        if line.startswith('Files:'):
            ...
            files_size = self.convert_size(match['size'], match['size_unit'])
            self.completion_state.files_size = files_size
            self.completion_state.new_files = int(match['new_files'])
            new_files_size = self.convert_size(match['new_size'], match['new_size_unit'])
            ...
            chunks_size = self.convert_size(match['size'], match['size_unit'])
            self.completion_state.chunks_size = chunks_size
            self.completion_state.new_chunks = int(match['new_chunks'])
            new_chunks_size = self.convert_size(match['new_size'], match['new_size_unit'])
